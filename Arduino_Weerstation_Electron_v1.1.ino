/* Arduino Weerstation met NodeMCU versie 1.1
 * Met ingebouwde AUTO RESET
****************************************************************************
Concept weerstation - code geschreven door Koen Vervloesem en Martijn Overman van Reshift.nl
in Computer!Totaal april 2018
***************************************************************************
Arduino Weerstation gemaakt door Cor Struyk, 9 aug 2019
...
wijzig de regels : 41, 42, 44, 45, 47, 325/326 en 329 voor juiste keuze en gegevens
--------------------------------------------------------------------------
...
***************************************************************************
De sketch is voorzien van een automatische software reset om eventuele buffer
problemen in de NodeMCU te voorkomen. Auto reset volgt na 1 uur
****************************************************************************
*
...
const char* ssid = " hier je SSID invullen";
const char* password = " hier je Wifi wachtwoord invullen";
//
...
int reset_teller = 0;  // zet autoreset teller op 0
...
void checkweer() {
  ...
  tft.setTextSize(1);
  tft.setTextFont(2);  
  tft.setTextColor(TFT_WHITE);  // Instelling zwarte achtergrond en eventule tekst wissen
...
//  tft.setTextSize(2);  // Zet deze regel vrij bij langere naam weerstation !
    ...
    tft.setTextSize(2);
    lees_tot_tag("<temperatuurGC>", "</temperatuurGC>", " graden C ", 7);
...
void checkrain() {
  ...
  while(client.available() && inhoud==""){
    String regel = client.readStringUntil('\r');
    int begin_pos = regel.indexOf(tijd_begin_tag);
    if(begin_pos != -1) {
      int einde_pos = regel.indexOf(tijd_einde_tag);
      String inhoud = regel.substring(begin_pos + tijd_begin_tag.length(), einde_pos);
      nu = inhoud;
      Serial.print("Huidig tijdstip: ");
      Serial.println(nu);
    }
  }

  const char* rainhost = raindomain.c_str();  
  Serial.print("Verbind met ");
  Serial.println(rainhost);

  if (!client.connect(rainhost, httpPort)) {
    Serial.println("Verbinding mislukt");
    client.stop();
    return;
  }
  ...
  Serial.print("Vraag URL op: ");
  Serial.println(rainurl + '&' + randNumber);
  client.print(String("GET ") + rainurl + '&' + randNumber + " HTTP/1.1\r\n" +
                 "Host: " + rainhost + "\r\n\r\n");
...
void loop() {
  ...
  // autoreset teller ophogen en testen tot 12x 5 min = 1 uur
  reset_teller++;
  if(reset_teller >= 12) { 
  Serial.println(" nu reset");
  delay(1000); 
  ESP.restart();  
  }
